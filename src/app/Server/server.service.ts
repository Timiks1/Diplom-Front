...
getTeacherTests(): Observable<TeacherTest[]> {
  const teacherId = this.currentUserValue.userId; // ID учителя берется из currentUser
  return this.http.get<any>(`${this.baseUrl}/TeacherTests/GetByUserId/${teacherId}`)
    .pipe(
...
addTeacherTest(testTheme: string, testUrl: string, group: string): Observable<any> {
  const teacherId = this.currentUserValue.userId;
...
updateTeacherTest(test: TeacherTest): Observable<any> {
  return this.http.put<any>(`${this.baseUrl}/TeacherTests/${test.id}`, test).pipe(
    catchError(error => {
...
getDepartmentMeetings(departmentId: string): Observable<any[]> {
  return this.http.get<any>(`${this.baseUrl}/DepartmentMeetingPlans/GetByDepartmentId/${departmentId}`).pipe(
...
downloadTemplate(): Observable<any> {
  return this.http.get<any>(`${this.baseUrl}/ExchangeVisitsPlans/4d82beb4-5e7b-48e6-b084-5bdc485bc1e7`).pipe(
    catchError(error => {
...
uploadExchangeVisitFile(file: File, teacherId: string, fileName: string): Observable<any> {
  const formData = new FormData();
  formData.append('file', file, fileName);
  ...
  return this.http.post<any>(`${this.baseUrl}/ExchangeVisitsPlans`, formData, { params }).pipe(
    catchError(error => {
